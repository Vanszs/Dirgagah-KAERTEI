...
main() {
    ...
    test_start "26 checkpoint system"
    if grep -q "checkpoint.*26" competition_startup.sh 2>/dev/null; then
        test_pass "26-checkpoint system configured"
    else
        test_warning "Cannot verify 26-checkpoint configuration"
    fi
