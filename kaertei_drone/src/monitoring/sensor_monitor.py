...
class SensorMonitorNode(Node):
    ...
    def read_sensors_callback(self):
        ...
        try:
            ...
            status_msg.distance_left = distance_left
            status_msg.distance_right = distance_right
            status_msg.distance_front = distance_front
            status_msg.is_centered = is_centered
            
            # Determine status
            if distance_front < self.get_parameter('distance_threshold').value:
                status_msg.status = "OBSTACLE_FRONT"
            elif not is_centered:
                if distance_left < distance_right:
                    status_msg.status = "DRIFT_LEFT"
                else:
                    status_msg.status = "DRIFT_RIGHT"
            else:
            ...
            # Publish individual distance readings
            left_msg = Float32()
