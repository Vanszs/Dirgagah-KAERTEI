...
from mavros_msgs.srv import CommandBool, SetMode, WaypointPush, WaypointPull, WaypointGOTO
...
class PX4WaypointNavigator(Node):
    ...
    def __init__(self):
        ...
        self.waypoint_goto_client = self.create_client(WaypointGOTO, '/mavros/mission/goto')
    ...
    def goto_px4_waypoint(self, waypoint_index):
        ...
        # Use GOTO command untuk navigate ke specific waypoint
        if not self.waypoint_goto_client.wait_for_service(timeout_sec=5.0):
            self.get_logger().error("❌ Waypoint GOTO service not available")
            return False
        
        request = WaypointGOTO.Request()
        request.wp_seq = waypoint_index
        
        try:
            future = self.waypoint_goto_client.call_async(request)
            rclpy.spin_until_future_complete(self, future, timeout_sec=10)
            
            if future.result().success:
                self.get_logger().info(f"✅ Successfully commanding PX4 to go to waypoint {waypoint_index}")
                self.waypoint_status = PX4WaypointStatus.GOING_TO_WAYPOINT
                return True
            else:
                self.get_logger().error(f"❌ Failed to command PX4 to go to waypoint {waypoint_index}")
                return False
        except Exception as e:
            self.get_logger().error(f"❌ Waypoint GOTO service error: {e}")
            return False
