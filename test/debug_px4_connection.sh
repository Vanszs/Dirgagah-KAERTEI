...
# Foxy 'ros2 topic echo' has no '-n/--once'; sample via timeout + awk
set +o pipefail
CONNECTED=$(timeout 5s ros2 topic echo /mavros/state 2>/dev/null | awk '/connected:/ {print $2; exit}')
MODE=$(timeout 5s ros2 topic echo /mavros/state 2>/dev/null | awk '/mode:/ {print $2; exit}')
set -o pipefail
...
set +o pipefail
timeout 2s ros2 topic echo /mavros/heartbeat 2>/dev/null | sed -n '1,50p' || true
set -o pipefail
